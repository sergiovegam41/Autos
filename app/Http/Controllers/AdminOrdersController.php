...
//			$this->col[] = ["label"=>"Aplicar iva","name"=>"tax","callback_php"=>'number_format($row->tax)'];
        ...
        $this->col[] = ['label' => 'Fecha de entrega', 'name' => 'delivery_date_time'];
...
//        }
        ...
        $this->form[] = ['label'=>'Fecha De Entrega','name'=>'delivery_date_time','type'=>'datetime','width'=>'col-sm-10'];
...
//                    $this->form[] = ['label'=>'Numero de Orden','name'=>'order_number','type'=>'text','validation'=>'required|min:1|max:255','value'=>$order_number,'readonly'=>true];
                ...
                if (CRUDBooster::myPrivilegeId() == '3' || CRUDBooster::isSuperadmin()) {
                    ...
                    $inventory = [];
                    $inventory[] = ['label' => 'VIN', 'name' => 'imei', 'type' => 'text'];
...
//            $this->form[] = ['label' => 'IVA ' . $iva . "%", 'name' => 'is_fax', 'type' => 'checkbox', 'width' => 'col-sm-10', 'dataenum' => "1|APLICAR"];
        ...
        */
        ...
        if (CRUDBooster::isSuperadmin() || $id_cms_privileges == '3') {
            ...
            $this->addaction[] = [
                'label' => 'Entregada-!-',
                'url' => CRUDBooster::mainpath('set-status/entregar/[id]'),
                'icon' => 'fa fa-truck',
                'color' => 'success',
                'showIf' =>  "[status] !=  $pendiente && [status] != $cancelada && [status] != $incompleta && [status] != $entregado",
            ];
                ...
                let list = document.querySelectorAll('td');
                    ...
                    list.forEach(function (item){
                      ...
                      if ( item.innerText.includes(mesActual) ) {
                         ...
                         item.setAttribute('style', 'background: #A8DAF899 !important;box-shadow: 0 0px 0px #A8DAF899  !important;color:black;font-weight:200;');
                         item.innerText += ' *Este Mes* - ';
                      }

                      if ( item.innerText.includes(MesPasado) ) {

                         item.setAttribute('style', 'background: #F7F7A050 !important;box-shadow: 0 0px 0px #F7F7A050  !important;color:black;font-weight:200;');
                         item.innerText += ' *Mes pasado* - ';
                      }

                      if ( item.innerText.includes(fechaHoy) ) {

                         item.setAttribute('style', 'background: #F7A0A099 !important;box-shadow: 0 0px 0px #F7A0A099  !important;color:black;font-weight:200;');
                         item.innerText += ' *Hoy* ';
                      }
        ...
        if (CRUDBooster::getCurrentMethod() == "getAdd") {
            ...
            $this->script_js = "
                ...
                function calculate_discount() {
                    ...
                    if (quantity == 0 || sale_price == 0 || real_price == 0 ) {
...
//        $id_cms_privileges = DB::table('cms_users')->where('id', '=', CRUDBooster::myId())->select('id_cms_privileges')->first()->id_cms_privileges;
        ...
        if ( CRUDBooster::myPrivilegeId() == CmsUser::Coordinador ||  CRUDBooster::myPrivilegeId() == CmsUser::AsesorComercial ||  CRUDBooster::myPrivilegeId() == CmsUser::Tecnico) {
...
//            if($order->guide_number){
                $this->orderRepository->updateStatus($id, Orders::ENTREGADO);
                CRUDBooster::redirect($_SERVER['HTTP_REFERER'], $posData['message'], "info");
//            }else{
//                CRUDBooster::redirect($_SERVER['HTTP_REFERER'], "No puedes entregar la orden sin Numero de guia", "info");
//            }
