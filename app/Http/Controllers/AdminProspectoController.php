...
//			$this->form[] = ['label'=>'Numero de telefono','name'=>'phone_number','type'=>'number','validation'=>'numeric','width'=>'col-sm-10','placeholder'=>'Puedes introducir solo un número'];
            ...
            $this->form[] = ['label'=>'Contacto 1','name'=>'contact_1','type'=>'text','validation'=>'numeric|min:1','width'=>'col-sm-10','placeholder'=>'+57 3043387716'];
            $this->form[] = ['label'=>'Contacto 2','name'=>'contact_2','type'=>'text','validation'=>'numeric|min:1','width'=>'col-sm-10','placeholder'=>'+57 3234683992'];
	        ...
	        */
            ...
            $this->addaction[] = ['url' => CRUDBooster::mainpath('set-send-message-whatsapp/[id]'), 'label' => 'Actualizar en Whatsapp', 'icon' => 'fa fa-whatsapp', 'color' => 'success'];
...
//            $this->addaction[] = ['url' => CRUDBooster::mainpath('set-oportunidad/[id]'), 'label' => 'Convertir a Oportunidad', 'icon' => 'fa fa-dollar', 'color' => 'info'];
	        ...
	        */
	        $this->index_statistic = array();
                ...
                if(CRUDBooster::myPrivilegeId() == '2'  || CRUDBooster::myPrivilegeId() == CmsUser::Tecnico ){
...
//
//
//                    addTextInput('contact_1','code_contact_1')
//                    addTextInput('contact_2','code_contact_1')
//
//                    function addTextInput(className,request_name) {
...
//            $query->orderByRaw("CASE WHEN prospecto.status = 'Por Contactar' THEN 1 ELSE 2 END");
...
//            if (!CRUDBooster::isSuperadmin() && $id_cms_privileges != "3"  ) {
            if ( CRUDBooster::myPrivilegeId() == CmsUser::AsesorComercial || CRUDBooster::myPrivilegeId() == CmsUser::Tecnico ) {
                $query->where('prospecto.user_id',  CRUDBooster::myId())->where('prospecto.stores_id',  $store_id);
            }

            if(CRUDBooster::myPrivilegeId() == CmsUser::Coordinador){
                $query->where('prospecto.stores_id',  $store_id)->orderBy('prospecto.updated_at','desc')->orderBy('prospecto.created_at','desc');
            }
	    ...
	    */
	    public function hook_before_add(&$postdata) {
            ...
            if($postdata['contact_1'] && strlen($postdata['contact_1']) < 12 ) $postdata['contact_1'] = "57".$postdata['contact_1'];
            if($postdata['contact_2'] && strlen($postdata['contact_2']) < 12 ) $postdata['contact_2'] = "57".$postdata['contact_2'];

            $postdata['contact_1'] = str_replace("+", "", $postdata['contact_1']);
            $postdata['contact_2'] = str_replace("+", "", $postdata['contact_2']);

	        if(CRUDBooster::myPrivilegeId() == CmsUser::AsesorComercial || CRUDBooster::myPrivilegeId() == CmsUser::Tecnico || CRUDBooster::myPrivilegeId() == CmsUser::Coordinador ){

                // TODO IMPLEMENTAR POR FILRO DEL PREFIJO POR PAIS
                $contact_1 = str_replace(" ","",str_replace("+57","",$postdata['contact_1']));
                $contact_2 = str_replace(" ","",str_replace("+57","",$postdata['contact_2']));


               $prospecto_contact_1 = Prospectos::where('contact_1',$contact_1)->orWhere('contact_1',"+57".$contact_1)->first();
               $prospecto_contact_2 = Prospectos::where('contact_2',$contact_2)->orWhere('contact_2',"+57".$contact_2)->first();


                $prospecto = null;

                if ($prospecto_contact_1->contact_1 != $contact_1 && $contact_2 =! $prospecto_contact_2->contact_2){

                }
                elseif( $prospecto_contact_1 && $prospecto_contact_1->contact_1 == $contact_1){
                    $prospecto = $prospecto_contact_1;
                } elseif ( $prospecto_contact_2 && $prospecto_contact_2->contact_2 == $contact_2){
                    $prospecto = $prospecto_contact_2;
                }

                if( $prospecto && $prospecto->user_id != \crocodicstudio\crudbooster\helpers\CRUDBooster::myId() && $prospecto->user_id ){

                    $UserName = CmsUser::where('id', $prospecto->user_id)->first()->name;
                    return   CRUDBooster::redirect($_SERVER['HTTP_REFERER'], "Este prospecto pertenece a ".$UserName, "danger");

                }


                if( $prospecto->user_id == null && $prospecto ){
               ...
               $rules = [
                    'contact_1' => 'required',
                    'contact_2' => $postdata['contact_2'] != null?'':''
               ];
    ...
    public function getSetSendMessageWhatsapp($id)
    {

        try {

            $prospecto = Prospectos::findorfail($id);
            $name = CmsUser::getMyNameById($prospecto->user_id)??CmsUser::getMyName();
            $messaage = '¡Bienvenido! Le informamos que ha sido asignado al asesor *'. ($name) .'* para atender sus requerimientos. A partir de este momento, nos comunicaremos con usted a través de este medio. ¡Estamos a su disposición para brindarle la mejor atención posible!';
             if( BotWhatsApp::sendMensage($prospecto->contact_1,$messaage,CmsUser::getStoresId()) ){
                 CRUDBooster::redirect($_SERVER['HTTP_REFERER'], "Se concateno con el cliente correctamente. ¡Bien hecho!", "success");
             }else{
...
//        dd( $prospecto->user_id );
      ...
      $code = $instanceCode->getCodeByID( parse_str( $prospecto->user_id) );
