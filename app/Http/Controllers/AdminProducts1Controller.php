...
//            $this->col[] = ["label"=>"Fecha de de vencimiento","name"=>"expiration_date"];
            ...
            $this->col[] = ["label"=>"¿Publico?","name"=>"is_public","callback" => function ($row) {
                if ($row->is_public == 1) {
                    return 'Si';
                } else {
                    return 'No';
...
//                commission_sale comision del asesor o coordinador
                $this->form[] = ['label'=>'Comision Base','name'=>'commission_sale','width'=>'col-sm-10','validation'=>'min:0|max:100'];

                $this->form[] = ['label' => '¿Publico?', 'name' => 'is_public', 'type' => 'select2', 'width' => 'col-sm-10', 'dataenum' => "1|SI;0|NO"];
//                is_public
            }
            $product = Products::PRODUCT;
...
//			$this->form[] = ['label'=>'Descuento en porcentaje','name'=>'discount_kit','type'=>'number','width'=>'col-sm-10','validation'=>'min:0|max:100'];
            ...
            if( CRUDBooster::getCurrentMethod() == "getAdd" ){
             ...
             $this->form[] = ['label'=>'Precio de venta comercial','name'=>'commercial_sale_price','width'=>'col-sm-10','validation'=>'min:0|max:100'];
            }
