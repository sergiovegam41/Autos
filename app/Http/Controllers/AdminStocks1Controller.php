...
class AdminStocks1Controller extends \crocodicstudio\crudbooster\controllers\CBController
{
    ...
    public function cbInit()
    {
        ...
        if( CRUDBooster::isSuperadmin() ){
            ...
            $this->form[] = ['label' => 'Productos', 'name' => 'products_id', 'type' => 'select2', 'validation' => 'required|integer|min:0', 'width' => 'col-sm-10', 'datatable' => 'products,name',"datatable_where"=>"type = 'PRODUCT'",'datatable_format'=>"name,' - ',type"];
        ...
        $columns = [];
        $columns[] = ['label' => 'VIN', 'name' => 'imei', 'type' => 'text','help'=>'Si el producto no tiene imei digitar "0" ', 'validation' => 'unique:inventory.imei', 'required' => true];
                    ...
                    $('#btn-add-table-inventario').on({
                        mouseenter: function () {
                          if(  parseInt(document.getElementById('inventarioimei').value) == 0 ){
                           ...
                           document.getElementById('inventarioimei').value = '000000' + new Date().getTime() + Math.floor(Math.random() * 99);
