...
class Products extends Model
{
    ...
    public function recursosInfo(): HasMany
    {
        $request = Request();
        if($request->buscar){
           return $this->hasMany(Recursos::class, "products_id")->where('type', Recursos::INFORMACION_TYPE)->where(function ($query ) use ($request) {
    ...
    public function recursosQuestion(): HasMany
    {
        $request = Request();
        if($request->buscar) {
            return $this->hasMany(Recursos::class, "products_id")->where('type', Recursos::PREGUNTA_TYPE)->where(function ($query ) use ($request) {
                $query->where('recursos.Asunto', 'LIKE','%'.trim($request->buscar).'%')
                    ->Orwhere('recursos.Descripcion','LIKE','%'.trim($request->buscar).'%');
            });
        }else{
